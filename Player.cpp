...
Player::Player(IAddProjectile& projectiles) : projectiles_(projectiles) {
...
void Player::attack(ProjectileManager& projectiles) const {
...
void Player::process(ProjectileManager& projectiles, PickupManager& pickups) {
    ...
    std::lock_guard enemy_lock(projectiles.getEnemyMtx());
    ...
    std::lock_guard pickup_lock(pickups.getMutex());
