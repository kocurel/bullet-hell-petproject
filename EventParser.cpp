...
EventParser::EventParser(IAddProjectile& projectiles, IAddEvent& event_manager) : event_factory_(projectiles, event_manager) {
    ...
    if (std::filesystem::is_directory(directory_path)) {
        ...
        std::sort(files_.begin(), files_.end(), [](std::filesystem::path& file1, std::filesystem::path& file2) {return (file1.c_str() < file2.c_str()); });
    }
}
