...
public:
    ...
    /// each take a concrete event id as a single parameter. The event
    /// handlers can be any kind of callable object that has an
    /// `operator()` defined for a specific event id. Additionally a
    /// generic callable can also be provided that will be invoked for
    /// every event id. If both types of callables are provided, the
    ...
    /// id of the event and other information available at compile
    /// time.
    ///
    ...
    ///          iOS and Android.
    ///
    /// \param cursor Native system cursor id to display
    ///
    ...
    ///
    /// The id of the returned handle is `sf::WindowHandle`,
    /// which is a id alias to the handle id defined by the OS.
